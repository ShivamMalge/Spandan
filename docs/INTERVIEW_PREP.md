...
> ### Note: the figures in this file were audited and corrected
>
> Writing this file meant checking every cited figure against actual command
> output rather than against the documents, and that surfaced stale figures in
> `docs/FAILURE_MODES.md`, `python/spandan/gen/ASSUMPTIONS.md` and
> `docs/BENCH.md`. A full read-only audit followed — `docs/AUDIT.md` — and the
> must-fix items were applied. **The source documents and this file now agree**,
> and every figure here traces to `make eval` or the dated `make bench` run.
>
> Keep the story, because it will be asked: the drift was found by re-deriving
> each citation from output, not by a test. No test in this project checks a
> number printed in a Markdown file, which is why documentation was the one
> place the plausible-number pattern could still survive. It is
> `docs/BUILD_LOG.md`'s sixth entry of that pattern and the first found by an
> external audit rather than by a guard.
...
distinct entities — 4,819 bytes each in Rust, projecting 38.6 GB a month at an assumed
...
much worse memory story than 4,819 bytes.
...
cards in this generator are 100% never-seen-elsewhere, the flash sale is 56.3% new
by distinct card, and outages 0.9%. So a detector keying on "share of unseen cards" would separate
...
checks. Measured: 4,819 bytes per entity in Rust, 1,971 in Python, projecting 38.6 GB
and 15.8 GB per month at an assumed 8M distinct entities (`BENCH.md` §4). Unbuilt
...
**74.7% per event** against a configured 75%, and **43.7% per distinct card**,
measured on every build and recorded in `manifest.json` — and the
...
**100%**, `flash_sale` **56.3%** by distinct card (25.3% by event), outages **0.9%**
(`ASSUMPTIONS.md` §1.7a).
...
puts at ₹2.52L and whose cost in customer trust it does not model at all.
...
matters is p99, not throughput. Measured: **p99 11.6µs against Python's 52.4µs**,
with 199,925 events/s streaming against 37,191. A single thread covers roughly 17
...
win: **it costs 2.44× the memory per entity** — 4,819 bytes against 1,971 — projecting
38.6 GB versus 15.8 GB a month at 8M entities. Never the throughput table without the
...
Rust 2,560 MB and 4,819 bytes/entity, Python 1,047 MB and 1,971 bytes/entity.
...
The general principle I would state: at 11.6µs p99 with a 5.38× margin already
...
Streaming, which is the shape that matters for an inline control: **Python 37,191
events/s with p50 25.5µs and p99 52.4µs; Rust 199,925 events/s with p50 4.7µs and
p99 11.6µs.** 5.38× throughput, 4.52× better tail.

Batch scoring at six batch sizes: Rust wins every one, from 1.55× at batch=1 (19.00µs
vs 29.45µs per event) to about 4.5× at mid sizes (5.43µs vs 24.56µs at batch=1,000).
...
₹11,077 in the cost model, because outage traffic was declining anyway. **Do not read
...
**Bounded total memory.** Entities are never freed — 38.6 GB a month projected at 8M
...
**And one that was on this list and has been closed.** An earlier draft flagged
stale figures in `FAILURE_MODES.md`, `ASSUMPTIONS.md` and `BENCH.md` as an
eleventh exposure. A read-only audit (`docs/AUDIT.md`) enumerated them with
evidence and the must-fix items were applied, so the documents now agree with
`make eval` and with the dated `make bench` run. What remains worth rehearsing
is not the defect but the mechanism: it was caught by re-deriving each citation
from command output, because no test in this project reads a number out of a
Markdown file.

---

# 14. THE QUESTION THE AUDIT SAYS WILL BE ASKED

**Q151. Your own test asserts the flash-sale known-customer share is above 0.50.
Your shipped dataset measures 0.437. Explain.**

Both numbers are real, they measure different things, and the shipped stream is
inside the intended band. Here is the whole answer.

**The mixture is configured 75% known / 25% fresh, and per event it measures
74.7%** — recomputed directly from `data/*.jsonl.gz`, not read from a document.
That is the number the detector experiences, because the detector scores events.

**43.7% is the share of distinct *cards*, and that statistic falls with stream
length by construction.** Known cards are capped by the benign population that
actually transacts — 8,742 of the 9,000-card pool. The 25% fresh draw mints a
brand-new card every time it fires. Over 100 days and about 45 flash-sale
episodes, the fresh cards accumulate without limit while the known ones
saturate, so distinct flash-sale cards reach 9,968 — more than the entire benign
card pool — and the distinct-card known share is dragged down even though the
per-draw mixture never moved.

**So the 0.50 bound does not transfer.** It was calibrated against
`SMALL_CONFIG`: a 4-day stream with two flash-sale episodes, where fresh cards
have barely had time to accumulate. Applying a length-sensitive statistic's
threshold across a 25× longer stream compares two different quantities. The test
still passes on the fixture it was written for, and it still guards what it was
written to guard there.

**Is the control still valid? Yes, and the reasoning is the one in
`ASSUMPTIONS.md` §1.7.** The requirement is that novelty alone must not separate
a sale from an attack. Attacks are 100% novel. The flash sale is 56.3% novel by
distinct card and 25.3% by event. Neither figure is close to 100%, so a novelty
feature would still be doing real work rather than getting the separation free —
and the reason that matters at all is that **no novelty feature exists**, a ban
enforced behaviourally by `test_no_card_novelty_state_is_retained` and
structurally by `Axis` having no `Card` variant.

**What I would concede.** The guard now runs only against the small fixture, so
nothing asserts this property against the data every published number comes
from. The right fix is an assertion against `manifest.json`'s
`negative_case.flash_sale_known_customer_share`, with a bound chosen for the
per-event share rather than the distinct-card one. It was not built, because the
detector and generator are frozen and this is a code change days before
submission. It is recorded in `docs/AUDIT.md` as H3, and the realised figures are
now stated in `ASSUMPTIONS.md` §1.7 so the number is at least written down where
someone would look.
